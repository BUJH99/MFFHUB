...
-- Stable portrait/form records. Keep the legacy uniforms table below for
-- existing account references while new skill data targets an appearance id.
...
create table if not exists public.character_effects (
  id uuid primary key default gen_random_uuid(),
  character_id text references public.characters(id) on delete cascade,
  portrait_id text references public.character_appearances(id) on delete cascade,
...
create or replace view public.v_character_db_matrix
with (security_invoker = true)
as
select
...
create or replace view public.v_account_spec_matrix
with (security_invoker = true)
as
with users as (
...
create or replace view public.v_alliance_battle_monthly_conditions
with (security_invoker = true)
as
select
  r.id as rotation_id,
...
-- The statements above are retained for legacy compatibility. These immutable
-- catalog objects deliberately expose SELECT only; ingestion runs as the
-- database owner/service role and does not rely on a public write policy.
